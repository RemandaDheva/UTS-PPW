...
if(isset($_GET['id'])) {
    ...
    if($result) {
        ...
        if($webtoon) {
            ...
            $illustrator = $webtoon['gambar'];
            $penerbit = $webtoon['penerbit'];
            $sinopsis = $webtoon['sinopsis'];
            $gambar = $webtoon['nama_gambar']; 
        }
    } else {
        ...
        if ($gambar['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($extension, $allowed_extensions)) {
                ...
                if (move_uploaded_file($gambar['tmp_name'], $target_file)) {
                    ...
                    $query = "UPDATE webtoon SET judul = ?, genre = ?, penulis = ?, gambar = ?, penerbit = ?, sinopsis = ?, nama_gambar = ? WHERE id_webtoon = ?";
                    $stmt = mysqli_prepare($koneksi, $query);
                    mysqli_stmt_bind_param($stmt, "sssssssi", $judul, $genre, $penulis, $illustrator, $penerbit, $sinopsis, $filename, $id_webtoon);

                    if (mysqli_stmt_execute($stmt)) {
                        $success_msg = "Detail manhwa berhasil diperbarui!";
                    } else {
...
<head>
    ...
    <title>WEBTOONZ - Edit Manhwa</title>
    ...
    <header id="header" class="header d-flex align-items-center fixed-top">
        <div class="container-fluid d-flex align-items-center justify-content-between">
            ...
            <div class="header-social-links">
                <a href="detail_manhwa.php?id=<?php echo $id_webtoon; ?>" class="search"><i class="bi bi-x-lg"></i></a>
            </div>
        </div>
    </header>
        ...
        <section class="insert" id="insert">
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="gambar" id="gambar" class="form-control" value="<?php echo $illustrator; ?>" required>
                </div>
                <div class="mb-3">
                    <label for="penerbit" class="form-label">Penerbit</label>
                    <input type="text" name="penerbit" id="penerbit" class="form-control" value="<?php echo $penerbit; ?>" required>
                </div>
                <div class="mb-3">
                    <label for="sinopsis" class="form-label">Sinopsis</label>
                    <textarea name="sinopsis" id="sinopsis" class="form-control" rows="5" required><?php echo $sinopsis; ?></textarea>
                </div>
                <div class="mb-3">
                ...
                <button type="submit" class="btn btn-primary">Perbarui Manhwa</button>
            </form>
        </section>
    </main>
